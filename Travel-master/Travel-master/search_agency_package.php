<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $stmt = $pdo->prepare('SELECT * FROM packages WHERE agency_id = :agency_id AND (location LIKE :search LIKE :search OR place_details LIKE :search OR num_days LIKE :search OR num_nights LIKE :search OR budget_price LIKE :search) AND package_status = :package_status');
...
<div class="jumbotron jumbotron-fluid package">
  <div class="container">
    <h1 class="display-4 text-primary font-weight-bold">Tìm kết quả: <?php echo $package_count; ?></h1>
    <p class="lead">
            ...
            echo '<div class="col-md-8">';
                echo '<div class="card-body pt-0">';
                    echo '<div>';
                        ...
                        echo '</h4>';
                        echo '<h5 class="font-italic text-info" style="font-size: .85rem;"><span class="mr-1"><i class="fas fa-map-marker-alt"></i></span>'. $package['location'];'</h5>';
                        echo '<div class="" style="font-size: .85rem;">
                                <p class="text-muted pt-2"><span class="mr-1" ><i class="far fa-clock"></i></span>'. $package['num_days'] .' ngày  '. $package['num_nights'] .' đêm</p>';
                    echo '</div></div>';
                    echo '<p class="card-text mb-3" style="font-size: .7rem;">'. substr($package['place_details'], 0, 40) .'...</p><hr>';
                    echo '<div>';
                        echo '<p class="text-muted" >
                                 <h5> '. number_format($package['budget_price'], 0 , $c = "," , $d = "." )."VND". '</h5>
